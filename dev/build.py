...
def build_hammer5_tools():
    """Builds the Hammer 5 Tools executable using PyInstaller."""
    subprocess.run([
...
def build_updater():
    """Builds the Hammer 5 Tools Updater executable using PyInstaller."""
    subprocess.run([
...
def create_installer(folder_path, output_exe):
    """Creates a self-extracting installer using Python, embedding the ZIP archive with a Tkinter GUI interface."""
    # Paths
    ...
    # Installer script content with Tkinter GUI
    installer_script = f"""
import sys
import os
import base64
...
def main():
    ...
    default_install_dir = os.path.expandvars('%PROGRAMFILES%\\Hammer5Tools')
    ...
    intro_label = ttk.Label(frame, text='Welcome to the Hammer 5 Tools Installer')
    ...
    install_button.grid(row=3, column=0, columnspan=3, pady=(10, 0))
...
if __name__ == '__main__':
    ...
    try:
        ...
        subprocess.run([
            'pyinstaller', '--onefile', '--noconfirm', '--name', os.path.splitext(os.path.basename(output_exe))[0],
            '--distpath', os.path.dirname(output_exe), temp_installer_path
        ], check=True)
...
def main():
    ...
    parser.add_argument('--installer', action='store_true', help="Create an installer using Python SFX with Tkinter GUI.")
