...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int)
{
    ...
    auto guiExecutable = appRoot / "Hammer5Tools_Core.exe";
    if (!fs::exists(guiExecutable))
        guiExecutable = installRoot / "Hammer5Tools_Core.exe";
    if (!fs::exists(guiExecutable))
    {
        ReleaseMutex(mutex);
        CloseHandle(mutex);
        MessageBoxW(nullptr, L"Hammer5Tools_Core.exe is missing from the application root.",
